...
#include <broken_pipeline/schedule/detail/coro_resumer.h>
...
TaskCoro AwaitOnce(std::shared_ptr<detail::CoroAwaiter> awaiter, ReadyQueue& queue,
                   bool& finished) {
...
TEST(CoroResumerTest, Basic) {
  auto resumer = std::make_shared<detail::CoroResumer>();
...
TEST(CoroAwaiterTest, SingleWaitFirst) {
  ...
  ASSERT_OK_AND_ASSIGN(auto awaiter,
                       detail::CoroAwaiter::MakeCoroAwaiter(/*num_readies=*/1, resumers));
...
TEST(CoroAwaiterTest, AnyWaitFirst) {
  ...
  for (auto& resumer : resumers) {
    resumer = std::make_shared<detail::CoroResumer>();
  }
  ASSERT_OK_AND_ASSIGN(auto awaiter,
                       detail::CoroAwaiter::MakeCoroAwaiter(/*num_readies=*/1, resumers));
...
TEST(CoroAwaiterTest, AnyReentrantWait) {
  ...
  resumers[lucky] = std::make_shared<detail::CoroResumer>();
...
TEST(CoroAwaiterTest, AllWaitFirst) {
  ...
  ASSERT_OK_AND_ASSIGN(
      auto awaiter,
      detail::CoroAwaiter::MakeCoroAwaiter(/*num_readies=*/num_resumers, resumers));
